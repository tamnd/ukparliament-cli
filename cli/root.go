// Package cli assembles the ukparliament command tree from the ukparliament
...
// NewApp assembles the kit application from the ukparliament domain. The
...
//
// To add a command, declare it in ukparliament/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := ukparliament.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(ukparliament.Domain{}).Register(app)
