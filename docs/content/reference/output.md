...
describes what users get. Pick a format with `-o`, or let ukparliament choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ukparliament <command> -o table   # aligned columns for reading
ukparliament <command> -o jsonl   # one JSON object per line, for piping
ukparliament <command> -o json    # a single JSON array
ukparliament <command> -o csv     # spreadsheet friendly
ukparliament <command> -o tsv     # tab-separated
ukparliament <command> -o url     # just the URL column
ukparliament <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ukparliament <command> --fields id,title,url
```
...
```bash
ukparliament <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ukparliament <command>            # a table, because this is a terminal
ukparliament <command> | wc -l    # JSONL, because this is a pipe
```
